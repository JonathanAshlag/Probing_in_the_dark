...
def parse_args():
    ...
    parser.add_argument("--save_dir", type=str, default="runs_pendulum_modern", help="the dir to save the model")
    ...
    parser.add_argument("--total_timesteps", type=int, default=1500000, help="total timesteps of the experiments")
    ...
    parser.add_argument("--dreamer_batch_size", type=int, default=64, help="the dreamer batch size")
    ...
    parser.add_argument("--warm_up", type=int, default=20, help="the number of warm-up iterations (computed in runtime)")
    ...
    parser.add_argument("--knn_k", type=int, default=5, help="the number of nearest neighbors to use in the KNN for the dynamics model")
...
def make_env(env_id, idx, capture_video, run_name, gamma,horizon=100):
    def thunk():
...
if __name__ == "__main__":
    ...
    run_name = f"{args.env_id}__{args.seed}_entropy_type_{args.entropy_type}_{args.dm}_KNN_{args.knn_k}_alpha{args.ent_coef}_10mil"
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        if warmup_finished:
            ...
            for i in range(args.num_envs):
                obs_entropy = utils.compute_state_entropy(src_feats=unnormed_obs[args.free_start:,i], tgt_feats=unnormed_obs[args.free_start:,i], average_entropy=False,batch=False,calc_steps=args.entropy_sub_calc,k_fixed=args.knn_k)
                state_entropy = utils.compute_state_entropy(src_feats=unnormed_true_states[args.free_start:,i], tgt_feats=unnormed_true_states[args.free_start:,i], average_entropy=False,batch=False,calc_steps=args.entropy_sub_calc,k_fixed=args.knn_k)
                latent_entropy = utils.compute_state_entropy(src_feats=latents[args.free_start:,i], tgt_feats=latents[args.free_start:,i], average_entropy=False,batch=False,calc_steps=args.entropy_sub_calc,k_fixed=args.knn_k)
            ...
            with torch.no_grad():
                ...
                if args.advantage_type=='gae':#gae
                    ...
                    for t in reversed(range(args.num_steps)):
                        ...
                        rewards = rewards.clone()#we will use the rewards as the baseline
