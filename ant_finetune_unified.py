...
def parse_args():
    ...
    parser.add_argument("--save_dir", type=str, default="runs/state/ant_finetune", help="the dir to save the model")
    ...
    parser.add_argument("--update_epochs", type=int, default=8, help="the K epochs to update the policy")
    ...
    parser.add_argument("--dm", type=str, default="contextual", help="the type of the dreamer to use, either 'base' or 'contextual")
    ...
    parser.add_argument("--pretrain_type", type=str, default="latent", help="the type of pretraining to use, options are: 'latent', 'obs', 'state, random'")
    ...
    parser.add_argument("--task", type=str, default="jump", help="the task to use in the ant environment")
    parser.add_argument("--reward_threshold", type=float, default=3.0, help="the hight threshold to consider the task as solved")
...
def make_env(env_id, idx, capture_video, run_name, gamma,horizon=1000,po_threshold=0.75,include_cfrc_ext_in_observation=False,task="jump",mean=None,var=None,count=None,norm_reward=True):
    def thunk():
        if capture_video and idx == 0:
            env = gym.make(env_id, render_mode="rgb_array")
            env = gym.wrappers.RecordVideo(env, f"videos/{run_name}")
        else:
            env = gym.make(env_id,po_threshold=po_threshold,include_cfrc_ext_in_observation=include_cfrc_ext_in_observation,task=task,success_truncation=True)
...
if __name__ == "__main__":
    ...
    run_name = f"{args.env_id}__{args.seed}_pretrain_type_{args.pretrain_type}_num_envs_{args.num_envs}_ent_coef_{args.ent_coef}_lr_{args.learning_rate}_{args.reward_threshold}_online_finetune"
    ...
    envs = gym.vector.SyncVectorEnv(
        [make_env(args.env_id, i, args.capture_video, run_name, args.gamma,args.num_steps,args.po_threshold,args.include_cfrc_ext_in_observation,args.task,mean=norm_mean,var=norm_var,count=norm_count,norm_reward=True) for i in range(args.num_envs)]
    )
    eval_envs = gym.vector.SyncVectorEnv(
        [make_env(args.env_id, i, args.capture_video, run_name, args.gamma,args.num_steps,args.po_threshold,args.include_cfrc_ext_in_observation,args.task,mean=norm_mean,var=norm_var,count=norm_count,norm_reward=False) for i in range(args.num_envs)]
    )
    ...
    # switch_baseline = utils.MovingAvgRobust(k=20,tensor_shape=(args.num_steps,),device=device)
    ...
    # if args.entropy_type in ["latent", "feat"] and args.swicth <args.num_iterations:
    #     entropy_type = "obs"
    # else:
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for step in range(0, args.num_steps):
            ...
            #     if unnormed_true_states[:,:,0].max().item() > highest_reached:
            #         highest_reached = unnormed_true_states[:,:,0].max().item()

            #     writer.add_scalar("charts/highest_reward", highest_reached, global_step)
        ...
        with torch.no_grad():
            ...
            if args.advantage_type=='gae':#gae
                ...
                for t in reversed(range(args.num_steps)):
                    ...
                    # curr_baseline= 0.95*curr_baseline#soften the baseline
