...
class AntEnv(MujocoEnv, utils.EzPickle):
    ...
    * v0: Initial versions release
    ...
    def __init__(
        ...
        po_threshold: float = 0.75,
        success_truncation: bool = False,
        task: str = "jump",
        camera_mode: str = "track_all",  # "track_all", "track_xy", or "fixed"
        fixed_camera_height: float = 0.5,  # Height for xy tracking mode
        **kwargs,
    ):
        # Use local XML file if not specified
        if xml_file is None:
            xml_file = os.path.join(os.path.dirname(__file__), "ant.xml")
        ...
        self.po_threshold = po_threshold
        self.task = task # "jump" or "escape"
        ...
        self.full_state_shape =(obs_size,)
    ...
    def _apply_partial_observability(self, observation):
        """Apply partial observability masking when z > threshold."""
        obs = observation.copy()
        
        # Determine indices based on observation structure
        if self._exclude_current_positions_from_observation:
            # When x,y excluded: z is at index 0, qvel starts at index 11 (13-2)
            z_pos_idx = 0
            z_orient_idx=4
            qvel_start = 13  # qpos size (13) - 2 excluded
            z_vel_idx = qvel_start + 2  # z velocity is 3rd element in qvel
            z_angular_vel_idx = qvel_start + 5  # z angular velocity is 6th element in qvel
        else:
            # When x,y included: z is at index 2, qvel starts at index 13
            z_pos_idx = 2
            z_orient_idx=6
            qvel_start = 15
            z_vel_idx = qvel_start + 2
            z_angular_vel_idx = qvel_start + 5  # z angular velocity is 6th element in qvel
        
        # Set z coordinate to threshold
        obs[z_pos_idx] = self.po_threshold
        # Set z velocity to 0
        obs[z_vel_idx] = 0.0
        # Set z orientation (quaternion) to no rotation
        obs[z_orient_idx] = 0.0
        #set z angular velocity to 0
        obs[z_angular_vel_idx] = 0.0
        
        # If including contact forces, also zero out z-related contact forces
        if self._include_cfrc_ext_in_observation:
            cfrc_start = qvel_start + 14  # After all qvel elements
            # Each body has 6 contact force elements (fx, fy, fz, tx, ty, tz)
            # Zero out fz (index 2) and tz (index 5) for each body (13 bodies total, excluding world)
            for body_idx in range(13):
                base_idx = cfrc_start + body_idx * 6
                obs[base_idx + 2] = 0.0  # fz
                obs[base_idx + 5] = 0.0  # tz
    ...
    def step(self, action):
        ...
        if self.task == "jump":
            ...
            reward = self.escape_reward(observation)
        else:
        ...
        info['state'] = observation.copy()
        
        # Apply partial observability if z height > threshold
        if self.data.qpos[2] > self.po_threshold:
            observation = self._apply_partial_observability(observation)

        if reward == 1:
            done = True if self.success_truncation else False
        else:
            done = False
    ...
    def render(self):
        """Override render to implement custom camera modes."""
        import mujoco

        if self.camera_mode == "track_xy" and self.render_mode == "rgb_array":
            # Custom rendering for track_xy mode
            if hasattr(self, 'data') and hasattr(self, 'model'):
                # Get ant position
                ant_x = self.data.qpos[0]
                ant_y = self.data.qpos[1]

                # Create a custom camera matrix for rendering
                # We'll render directly using mujoco's rendering functions
                width = 640
                height = 480

                # Create renderer if needed
                if not hasattr(self, '_custom_renderer'):
                    self._custom_renderer = mujoco.Renderer(self.model, height, width)

                # Set up camera for side view before updating scene
                camera = mujoco.MjvCamera()
                camera.lookat[0] = ant_x  # Follow ant x position
                camera.lookat[1] = ant_y  # Follow ant y position
                camera.lookat[2] = 0.5    # Look at ant center height
                camera.distance = 4.0     # Distance from ant
                camera.azimuth = 0        # Side view (0 degrees from x-axis)
                camera.elevation = 0      # Horizontal view (perpendicular to ground)

                # Update renderer with current data and camera
                self._custom_renderer.update_scene(self.data, camera=camera)

                # Render and return
                return self._custom_renderer.render()

        elif self.camera_mode == "fixed" and self.render_mode == "rgb_array":
            ...
            if hasattr(self, 'data') and hasattr(self, 'model'):
                ...
                # Set up fixed side view camera (perpendicular to ground)
                camera = mujoco.MjvCamera()
                camera.lookat[0] = 0      # Look at center
                camera.lookat[1] = 0      # Look at center
                camera.lookat[2] = 0.5    # Look at center height
                camera.distance = 4.0     # Distance from center
                camera.azimuth = 0        # Side view (0 degrees from x-axis)
                camera.elevation = 0      # Horizontal view (perpendicular to ground)
    ...
    def reset(self, *, seed: int | None = None, options: dict | None = None):
        # 1) Let MujocoEnv handle seeding, TimeLimit, etc.
        obs, info = super().reset(seed=seed, options=options)
        if self.task =="escape":
            inital_pos =np.array([0,0, 0.2841, -0.1455,  0.5901, -0.7919, -0.0602, -0.2085,  0.4855, -0.0676,
         -1.1468, -0.5293, -0.9839,  0.4542,  1.1999, -0.0556])
            init_vel = np.zeros((1,14))
            self.set_state(inital_pos[:15], init_vel[0,:])
            obs = self._get_obs()

        # 2) Now `obs` is whatever reset_model() returned. Inject your "state" key and add noise:
        info["state"] = obs.copy()

        # Apply partial observability if z height > threshold
        if self.data.qpos[2] > self.po_threshold:
            obs = self._apply_partial_observability(obs)
    ...
    # def escape_reward(self,obs):
    #     l1 = self.get_body_com('aux_1')[2]
    #     l2 = self.get_body_com('aux_2')[2]
    #     l3 = self.get_body_com('aux_3')[2]
    #     l4 = self.get_body_com('aux_4')[2]
    #     thresh = 0.8
    #     is_upside_down = self.is_ant_upside_down_robust(obs)
    #     if l1 >= thresh and l2 >= thresh and l3 >= thresh and l4 >= thresh and not is_upside_down:
    #         return 1
    #     else:
    #         return 0
    def escape_reward(self, observation):
        """Simplified stability check."""
        if self._exclude_current_positions_from_observation:
            z_pos = observation[0]
            w, x, y, z_quat = observation[1:5]
        else:
            z_pos = observation[2]
            w, x, y, z_quat = observation[3:7]
        
        # Normalize and check full orientation
        norm = np.sqrt(w*w + x*x + y*y + z_quat*z_quat)
        if norm > 0:
            w, x, y, z_quat = w/norm, x/norm, y/norm, z_quat/norm
        
        up_z = 1.0 - 2.0 * (x*x + y*y)
        
        # Success: body pointing up + off the ground
        success = (up_z > 0.8) and (1 >z_pos > 0.5)
...
gymnasium.register(
    id="PO_ant-v5",
    entry_point="PROBE.po_antv5:AntEnv",
    )    
