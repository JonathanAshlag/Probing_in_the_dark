...
def parse_args():
    ...
    parser.add_argument("--dm", type=str, default="contextual", help="the type of the dreamer to use, either  'base' or 'contextual' or 'verybad_style'")
...
def make_env(env_id, idx, capture_video, run_name, gamma,horizon=100,noise_factor=0.05):
    def thunk():
        if capture_video and idx == 0:
            env = gym.make(env_id, render_mode="rgb_array")
            env = gym.wrappers.RecordVideo(env, f"videos/{run_name}")
        else:
            env = gym.make(env_id,noise_factor=noise_factor)
...
if __name__ == "__main__":
    ...
    run_name = f"{args.env_id}__{args.seed}_entropy_type_{args.entropy_type}_{args.noise_factor}_{args.dm}_KNN_{args.knn_k}_alpha{args.ent_coef}_10mil"
    ...
    envs = gym.vector.SyncVectorEnv(
        [make_env(args.env_id, i, args.capture_video, run_name, args.gamma,args.num_steps,args.noise_factor) for i in range(args.num_envs)]
    )
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        stats["prev_obs"] = torch.zeros_like(unnormed_ob)  # Initialize prev_obs with zeros for first step
        ...
        for step in range(0, args.num_steps):
            ...
            stats["prev_obs"] = unnormed_ob  # Update prev_obs to current obs for next iteration
