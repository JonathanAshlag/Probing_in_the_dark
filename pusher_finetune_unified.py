...
def parse_args():
    ...
    parser.add_argument("--env_id", default="PO_Pusher", help="the id of the environment")
    ...
    parser.add_argument("--dm", type=str, default="contextual", help="the type of the dreamer to use, either 'dreamer' or 'base' or 'contextual")
    ...
    parser.add_argument("--task", type=str, default="hard_left", help="the task to use, options are: 'top_left', 'top_right', 'bottom_left', 'bottom_right'")
...
def make_env(env_id, idx, capture_video, run_name, gamma,horizon=100,noise_factor=0.05,task=None,mean=None,var=None,count=None,norm_reward=False):
    def thunk():
        
        env = gym.make(env_id,noise_factor=noise_factor, task=task,success_truncation=True)
...
if __name__ == "__main__":
    ...
    run_name = f"{args.env_id}__{args.seed}_pretrain_type_{args.pretrain_type}_{args.noise_factor}_task_{args.task}_online_finetune"
    ...
    envs = gym.vector.SyncVectorEnv(
        [make_env(args.env_id, i, args.capture_video, run_name, args.gamma,args.num_steps,args.noise_factor,args.task,mean=norm_mean,var=norm_var,count=norm_count,norm_reward=True) for i in range(args.num_envs)]
    )
    assert isinstance(envs.single_action_space, gym.spaces.Box), "only continuous action space is supported"
    eval_envs = gym.vector.SyncVectorEnv(
        [make_env(args.env_id, i, args.capture_video, run_name, args.gamma,args.num_steps,args.noise_factor,args.task,mean=norm_mean,var=norm_var,count=norm_count,norm_reward=False) for i in range(args.evaluation_episodes)]
    )

    if args.dm == "base":
        features_to_predict = [17,18,19]
        ireducible_error = envs.envs[0].ireducible_error
